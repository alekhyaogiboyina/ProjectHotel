<?php
    ...
    if(isset($_POST['bookings_analytics'])){
        ...
        if($frm_data['period']==1){
            $condition="WHERE datantime BETWEEN(NOW() -INTERVAL 30 DAY) AND NOW()";
        }
        else if($frm_data['period']==2){
            $condition="WHERE datantime BETWEEN(NOW() -INTERVAL 90 DAY) AND NOW()";
        }
        else if($frm_data['period']==3){
            $condition="WHERE datantime BETWEEN(NOW() -INTERVAL 1 YEAR) AND NOW()";
        }
        ...
        SUM(CASE WHEN booking_status='booked' AND arrival=1 THEN 1 END) AS `active_bookings`,
        COUNT(CASE WHEN booking_status='booked' AND arrival=1 THEN `trans_amt` END) AS `active_amt`,
